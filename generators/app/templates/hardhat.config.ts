...
task('generate-root-hash', 'Generates and prints out the root hash for the current allowlist', async () => {
  // Check configuration
  if (CollectionConfig.allowlistAddresses.length < 1) {
    throw 'The allowlist is empty, please add some addresses to the configuration.';
  }

  // Build the Merkle Tree
  const leafNodes = CollectionConfig.allowlistAddresses.map(addr => keccak256(addr));
  const merkleTree = new MerkleTree(leafNodes, keccak256, { sortPairs: true });
  const rootHash = '0x' + merkleTree.getRoot().toString('hex');

  console.log('The Merkle Tree root hash for the current allowlist is: ' + rootHash);
});

task('generate-proof', 'Generates and prints out the allowlist proof for the given address (compatible with block explorers such as Etherscan)', async (taskArgs: {address: string}) => {
  // Check configuration
  if (CollectionConfig.allowlistAddresses.length < 1) {
    throw 'The allowlist is empty, please add some addresses to the configuration.';
  }
  ...
  console.log('The allowlist proof for the given address is: ' + proof);
})
